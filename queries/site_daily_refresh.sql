-- JIRA: HAT-3917 - Site Daily Refresh Query
--Daily Insert Query to cover for WAD 14Day refresh


DROP TABLE IF EXISTS date_process_range;
CREATE TEMP TABLE date_process_range
AS
SELECT
...
WHERE website_date BETWEEN (SELECT min_processing_date FROM date_process_range) AND (SELECT max_processing_date FROM date_process_range)
;
...
WITH base_traffic_data AS (
    SELECT
        ...
        -- Keep individual fields for source determination
    ...
    WHERE website_activity_mst_date BETWEEN (SELECT min_processing_date FROM date_process_range) AND (SELECT max_processing_date FROM date_process_range)
...
WHERE website_activity_mst_date BETWEEN (SELECT min_processing_date FROM date_process_range) AND (SELECT max_processing_date FROM date_process_range)
  AND bill_line_traffic_ext.product_pnl_line_name in ('Websites and Marketing', 'Website Builder')
...
-- Assign wam_sessions=1 to only ONE row per session
...
-- Find top-ranked tracking code using CASE WHEN in rank order
top_ranked_extract AS (
    SELECT
        ...
        item_tracking_code_click_list,
        item_tracking_code_impression_list,
        -- Check for ALL tracking codes in rank order (based on TopITCs.csv)
        CASE
...
SELECT a.*, itc_grouping FROM final_output a 
LEFT JOIN (select itemtrackingcode as item_tracking_code, itcgrouping as itc_grouping from dev.ba_corporate.wam_itc_site group by 1,2) b 
